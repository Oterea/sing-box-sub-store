...
> INFO 组里可能混进几个**冷门地区的真节点**（🇦🇬 安提瓜和巴布达、🇸🇧 所罗门群岛…）。
> `rename.js` 的地区表只有 180 来个国家，认不出它们，跟公告一起丢掉了。
>
> 这不是 INFO 引入的问题——那些节点一直在被静默丢弃，只是以前看不见。对 `proxy`
> 也没有损失：区域过滤本来就只保留你勾选的那几个地区。数量很少，不单独处理。
