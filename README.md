...
告警排在最前面，因为 iOS 通知不展开时只显示前几行。它是 `produceArtifact`
抛异常来的 —— **机场跑路、换域名、被墙都长这样**，比流量数字更值得看一眼。

每个机场的文案是它自己写的公告原文，各家字段名都不一样（「剩余流量」
「距离下次重置剩余」「上次更新」…），脚本不去解析统一成表格 —— 机场改一个字
就会解析错或漏掉。只把机场名提成一行，省掉每行重复。
...
第一行是这次的结果，Bark 没返回 200 时会显示 `推送失败，Bark 返回 <状态码>`。

**只想看、不想吵手机**，地址后面加 `?peek=1`：

```
https://<你的域名>/<后端路径>/api/file/push-info?peek=1
```

数据照样是现拉的，只是不发推送，第一行显示 `仅查看，未推送`。
做两个主屏幕图标（一个推一个看）就够用了。
...
| `?peek=1` | — | URL 上的 query，不是脚本参数。只刷新和显示，不发推送 |
| `title=` | `机场流量` | 通知标题 |
