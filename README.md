...
拉不到就重试，**最多 3 次**。但**确定性的错误一次就放弃** ——
`RESOURCE_NOT_FOUND` 表示订阅名不存在，试一百次也还是不存在，
三条请求三条日志零收益。`NO_FLOW_INFO` 保留重试，限流和网络抖动有可能自愈。

```
⚠️ gone RESOURCE_NOT_FOUND（0.0s）              ← 只试了一次，不写次数
⚠️ flaky NO_FLOW_INFO（试了 3 次，0.4s）
```


重试很便宜 —— 一个几百字节的本机请求，不再是下载整个节点列表。
重试后成功的机场，名字后面会标「重试 N 次」。
