...
```
[0.8s]  mitch · 0.8s      剩余流量：96.28G / 100.00G
                          较上次：消耗 3.17 GB（6.0 小时前）
                          套餐到期：2026-10-12
[1.2s]  ⚠️ dead · 1.2s    订阅拉取失败（试了 3 次）
[2.1s]  tolink · 2.1s     剩余流量：69.52 GB
                          较上次：消耗 3.95 GB（6.0 小时前）
                          套餐到期：2026-10-17
[4.0s]  pei · 4.0s        剩余流量：95.00 GB
                          较上次：+22.68 GB（已重置或充值，6.0 小时前）
```
...
剩余变多时不会显示成「消耗了负数」，而是认出来是套餐重置或充值：

```
较上次：+22.68 GB（已重置或充值，6.0 小时前）
```

**基准只在定时任务那次更新。** 网页访问时 `$options` 带着请求信息，定时产出时
它是 `undefined` —— 用这个区分。所以白天随手点几下不会把基准冲掉，点开看到的
始终是「距离上次定时推送消耗了多少」。

第一次跑没有基准，那一行不显示，不会硬凑一句占地方。基准不存在时**任何一次跑
都会建**（包括手动点），否则从来不用定时的人永远看不到差值；已经有基准之后，
就只有定时那次会更新它。

> **这一行要从文案里抽数字。** 靠的是 `剩余流量[：:]\s*([\d.]+)\s*(TB|GB|MB|T|G|M)`，
> 支持 `73.47 GB`、`96.28G / 100.00G` 这些写法。抽不出来会显示
> `较上次：无法计算（没认出流量数字）` —— **不会静默消失**，机场哪天改了
> 文案你能一眼看见。
>
> 为了让这一个正则够用，`rename.js` 里响应头那条路的措辞也对齐成了「剩余流量」
> （原先是「已用流量」）—— 各家机场自己写的公告普遍就是这个词。
...
tolink · 0.6s · 重试 1 次          ← 第 2 次才成功
⚠️ pei · 60.0s                    ← 3 次都失败
   订阅拉取失败（试了 3 次）
```

耗时算的是**从第一次尝试到出结果的总时间**，也就是你实际等了多久。

> 最坏情况：一家机场每次都卡到超时，3 次就是 60 秒，而且整个页面要等它。
> 目前没给单家加超时上限 —— 真遇到了再说。
